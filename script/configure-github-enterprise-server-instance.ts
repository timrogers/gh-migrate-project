...
(async () => {
  ...
  const { isGitHubEnterpriseServer, gitHubEnterpriseServerVersion } =
    await getGitHubProductInformation(ghesOctokit);
  ...
  logger.info(`Seeding GHES ${gitHubEnterpriseServerVersion} instance, if required...`);
  ...
  const secretNamePrefix = `GHES_${major(gitHubEnterpriseServerVersion)}${minor(gitHubEnterpriseServerVersion)}_`;
  ...
  await encryptAndSetActionsSecret({
    ...
    repo: PROJECT_REPO,
    sodium,
    secretName: `${secretNamePrefix}ACCESS_TOKEN`,
  ...
  await encryptAndSetActionsSecret({
    ...
    repo: PROJECT_REPO,
    sodium,
    secretName: `${secretNamePrefix}BASE_URL`,
